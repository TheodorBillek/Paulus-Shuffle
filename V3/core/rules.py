...
class RowProgressionRule(BaseRule):
    ...
    def penalty(self, assignment, reverse, students_by_id, seats_by_id, table_pairs, history_pairs, config):
        last_rows: Dict[int, int] = config.get("last_rows", {})
        p = 0.0
...
class PositionalRule(BaseRule):
    ...
    def penalty(self, assignment, reverse, students_by_id, seats_by_id, table_pairs, history_pairs, config):
        ...
        p = 0.0
        for sid_str, bounds in constraints.items():
            sid = int(sid_str)
...
class VicinityRule(BaseRule):
    ...
    def penalty(self, assignment, reverse, students_by_id, seats_by_id, table_pairs, history_pairs, config):
        separation: Dict[str, List[int]] = config.get("separation_pairs", {})
        min_dist: float = config.get("min_distance", 2.0)
        p = 0.0

        def coords(seat_id: int):
            s = seats_by_id.get(seat_id)
            if not s:
                return None
            return (s["row_idx"], s["col_idx"] * 2 + (0 if s["side"] == "L" else 1))

        for sid_str, avoid_ids in separation.items():
            sid = int(sid_str)
            ...
            for avoid_id in avoid_ids:
                seat2 = assignment.get(int(avoid_id))
                if not seat2:
                    continue
                c2 = coords(seat2)
                if not c2:
...
class FrontRowsFirstRule(BaseRule):
    ...
    def penalty(self, assignment, reverse, students_by_id, seats_by_id, table_pairs, history_pairs, config):
        """Gap safety-net only — the hard seat restriction in the engine already
        guarantees front-row placement.  This catches any gap that slips through
        (e.g. disabled seats inside the selected rows)."""
        if not assignment:
            return 0.0
